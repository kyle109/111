...
public class QuerySpecialList {
    ...
    public static void main(String[] args) throws ClientProtocolException, IOException
    {
        ...
        reportObj.put("testName", "测试");
        reportObj.put("testAll", 0);
        reportObj.put("testFail", 0);
        reportObj.put("beginTime", (dateFormater.format(new Date())).toString());
    ...
    protected static void readSheet(String filepath, int sheetNum, Map reportObj, List<Map> rstList, XSSFWorkbook workbook) throws IOException {
        ...
        for (int j = 1; j <= row; j++) {
            ...
            if (a.equals("N")) {
                excelUtil.write(filepath, sheetNum, j, 11, "skip");
                requstMap.put("spendTime", "0ms");
                requstMap.put("status", "跳过");
                reportObj.put("testSkip", Integer.parseInt(reportObj.get("testSkip").toString()) + 1);
            } else {
                //动态获取预期结果
